...
// Function to threshold vegetation index
...
// --------------------------------------------
// Produce map in Google Earth's viewer
...
// =========================================
// Display a histogram of NDVI values for each season for a
// small box (region) in the middle of Ireland
/*
var region = ee.Geometry.Rectangle(-9, 53 , -7, 54)
var histogram1 = ui.Chart.image.histogram(ndvi_summer,region,10000);
var histogram2 = ui.Chart.image.histogram(ndvi_spring,region,10000);
var histogram3 = ui.Chart.image.histogram(ndvi_autumn,region,10000);
var histogram4 = ui.Chart.image.histogram(ndvi_winter,region,10000);
...
// =========================================
// save images of NDVI to Google drive


Export.image.toDrive({
  image: ndvi_winterout,
  description: 'WinterNDVI',
  scale: 500,  //m per pixel
  dimensions: "6x10", // WIDTHXHEIGHT
...
Export.image.toDrive({
  image: winter.select('R','G','B').median(),
...
Export.image.toDrive({
  image: spring.select('R','G','B').median(),
...
Export.image.toDrive({
  image: summer.select('R','G','B').median(),
...
Export.image.toDrive({
  image: autumn.select('R','G','B').median(),
